...
async def all_orders(db: Session = Depends(get_db), admin: UserPayload=Depends(require_admin)  ):
    return list_orders(db, admin.id )

@router.get("/{order_id}", response_model=OrderOut)
async def single_order(order_id: int, db: Session = Depends(get_db), current_user:UserPayload=Depends(get_current_user_payload)):
